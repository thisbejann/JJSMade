...
const QC_VISIBLE_STATUSES = new Set<PersonalItemStatus>([
  "qc_sent",
  "item_shipout",
...
const SHIPPING_VISIBLE_STATUSES = new Set<PersonalItemStatus>([
  "item_shipout",
  "arrived_ph_warehouse",
  "delivered_to_me",
]);
...
interface PersonalItemFormProps {
  existingItem?: Doc<"personalItems">;
...
function sanitizeSizeForCategory(category: ItemCategory, currentSize: string) {
  ...
  if (category === "clothes") {
    ...
    return CLOTHES_SIZES.includes(upper as (typeof CLOTHES_SIZES)[number]) ? upper : "";
  }
  if (!trimmed) return "";
...
export function PersonalItemForm({ existingItem, onSuccess }: PersonalItemFormProps) {
  const { settings } = useSettings();
  const createItem = useMutation(api.personalItems.create);
  ...
  const [exchangeRateTouched, setExchangeRateTouched] = useState(Boolean(existingItem));
  ...
  const [isForwarderBuy, setIsForwarderBuy] = useState(existingItem?.isForwarderBuy ?? false);
  ...
  const [forwarderBuyRateTouched, setForwarderBuyRateTouched] = useState(Boolean(existingItem));
  ...
  const [forwarderRateTouched, setForwarderRateTouched] = useState(Boolean(existingItem));
  ...
  const validateSize = () => {
    ...
    if (category === "clothes") {
      if (!CLOTHES_SIZES.includes(normalizedSize as (typeof CLOTHES_SIZES)[number])) {
        ...
        <CardContent className="space-y-4">
          ...
          {category === "clothes" && (
            <Select
              ...
              value={size}
              onChange={(e) => setSize(e.target.value)}
              options={CLOTHES_SIZES.map((itemSize) => ({ value: itemSize, label: itemSize }))}
            ...
            <>
              ...
              <div className="rounded-lg bg-accent-muted px-3 py-2">
                ...
                <p className="font-mono text-lg font-semibold text-accent">
                  PHP {costs.pricePHP.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </p>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="rounded-lg bg-surface px-3 py-2 border border-border-subtle">
                ...
                <p className="font-mono text-sm text-primary">
                  PHP {costs.localShippingPHP.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </p>
              </div>
            </>
          )}
          ...
          {isForwarderBuy && (
            <>
              ...
              <div className="rounded-lg bg-surface px-3 py-2 border border-border-subtle">
                ...
                <p className="font-mono text-sm text-primary">PHP {costs.qcServiceFeePHP.toFixed(2)}</p>
              </div>
            </>
          )}
        </CardContent>
          ...
          <CardContent className="space-y-4">
            ...
            {weightKg > 0 && (
              <div className="rounded-lg bg-surface px-3 py-2 border border-border-subtle">
                ...
                <p className="font-mono text-sm text-primary">
                  PHP {costs.forwarderFee.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                </p>
              </div>
            )}
          </CardContent>
