...
1. **`+ New` split control** in the Orders header — replaces the current `New Group` + `Add Item` pair (`src/pages/OrdersList.tsx:217-224`). **✅ CRAFTED** (`src/components/items/NewControl.tsx`): production split button on Radix `DropdownMenu` (keyboard nav, focus restore, Escape, click-outside built in); coral pill, main area `Add Item` → `/orders/new`, chevron menu = `Add Item` / `New Group Order` → `/calculator`. Each menu row carries a one-line descriptor ("Build & negotiate a bundle in the calculator") so the calculator destination isn't surprising. `NewGroupOrderModal` + old two-button pair removed from `OrdersList`. **RESOLVED via `/prototype` (variant A wins):** a **split button** — a coral pill whose main area is `Add Item` (one tap → `/orders/new`, the frequent action) plus a divided chevron segment that opens a menu listing `Add Item` and `New Group Order`. "New Group Order" routes to `/calculator`. Chosen over a single `+ New` dropdown because solo items are created far more often than group orders, so Add-Item-as-default earns the one-tap path. Craft: build it production-clean (keyboard nav, focus restore, Escape/click-outside), fold into `OrdersList`, and remove the old `NewGroupOrderModal`. Still to shape: how "New Group Order" reads so landing on the calculator isn't surprising.
2. **"Save as Group Order" CTA** on the Quote Calculator — appears when the Bundle is non-empty (`src/pages/QuoteCalculator.tsx`, bundle view ~line 335+). Placement relative to the existing negotiation panel + items list. **✅ CRAFTED** (`src/pages/QuoteCalculator.tsx`): full-width coral `Button size="lg"` with a trailing `ArrowRight01Icon`, placed below the items list inside the bundle view's `space-y-6` wrapper. Reading order is margin → items → commit. Coral is safe despite the one-warm-signal rule because `Add to bundle` lives in the separate Add-item segmented view, so the two primaries never share a screen; `Clear all` stays quiet ghost text. **Seam for surface 3:** the CTA toggles a stubbed `const [, setShowSheet] = useState(false)` flag carrying a `// TODO surface 3` comment — surface 3's session mounts the real Group Review Sheet on that flag (no navigation; the sheet is an overlay on this page). Shape decision (placement) resolved via an assert-then-confirm round; copy `Save as Group Order` is locked from CONTEXT.md, no sub-hint added.
3. **Group Review Sheet** (the main new surface) — full-height overlay: customer picker at top, one row per Bundle line (name · seller · size · read-only quote), editable negotiated total, Cancel / Create group. Must handle 1–8+ rows, mobile + desktop, and per-row validation (name/seller/size required before Create enables). This is the surface that most needs design care. **✅ CRAFTED** (`src/components/items/GroupReviewSheet.tsx`). **RESOLVED via `/prototype` (variant C wins):** a full-height overlay anchored to the calculator's `max-w-lg` column with a **progressive accordion** — one row expanded at a time, completing a row (name + seller + size where required) auto-advances to the next incomplete one. Header carries the `CustomerPicker` + a `{ready}/{total}` progress meter; each row shows a green check when complete; sticky footer holds the editable negotiated total (pre-filled from Offer Total), the "excl. forwarder shipping" caveat, and a coral `Create group order` button disabled until every row + a customer are valid (reads "{n} left to finish" until then). Size input is conditional: shoes = EU number, clothes = S/M/L/XL segmented, watches/accessories = none. Seller = real `Combobox` fed by `api.items.getUniqueSellers`. Backed by a single **`orderGroups.createWithItems`** mutation that seeds group + items + (conditional) `negotiatedTotal` in one transaction, re-using `items.ts`'s `validateItemRules`/`normalizeSize` + `computeDerivedFields` so seeded items match the calculator's cost; `negotiatedTotal`/`quotesSumAtEntry` persisted only when discounted. On Create → clears the Bundle and routes to `/groups/:id`; Cancel/scrim leaves the Bundle untouched. `BundleItem` gained `localShippingCNY` so lines seed real items. Prototype (`src/pages/ProtoGroupSheet.tsx` + guarded calculator block) deleted after the verdict was recorded.
